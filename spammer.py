...
def banner():
    ...
    print()
    print(r"     Creator: github.com/marselshow      ")
    print(RESET_ALL)


def main():
    banner()
    print("[1] Start BOMBER.")
    print("[2] Обновить.")
    print("[3] Exit.")
    print()
...
def spam_handler():
    ...
    threads = input(f"{BRIGHT}{BLUE}sugarbomber >> {RESET_ALL}")
    if threads in ["", " ", "0"]:
        threads = "1"
    try:
        if int(threads) > 10:
        ...
        threads = "1"
    banner()
    print(f"Номерок: {BRIGHT}{BLUE}{phone}{RESET_ALL}")
    print("Бомбер запущен", end="\n")
...
def parse_phone(phone):
    ...
    if len(phone) in [10, 11, 12]:
        ...
        print(f"\n{BRIGHT}{RED}[*] Номер не верно введён!{RESET_ALL}")
        sleep(1)
        spam_handler()
...
def check_internet():
    try:
        ...
        print(f"\n{BRIGHT}{RED}[*] Error connect to ethernet !{RESET_ALL}")
        sleep(1)
        main()
    return
...
def update():
    check_internet()
    banner()
    print("Желаете обновить бомбер? (y/n)")
    update = input(f"{BRIGHT}{BLUE}sugarbomber >> {RESET_ALL}")
    if update.lower() == "y":
